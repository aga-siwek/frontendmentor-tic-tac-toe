  ...
  effect: async (action, listenerApi) => {
    ...
    if (winData != null && state.gameState === GAME_STATE.IN_PROGRESS) {
      ...
      listenerApi.dispatch(
        gameSlice.actions.finishGame({ player: "", winCells: [] }),
      );
    }
  },
});
...
listenerMiddleware.startListening({
  matcher: isAnyOf(
    gameSlice.actions.makeMove,
    gameSlice.actions.startGame,
    gameSlice.actions.newRound,
  ),
  effect: (action, listenerApi) => {
    ...
    if (state.cpuPlayerActive) {
      ...
      const isCpuMove =
        (state.firstPlayer === "o" && state.isXTurn) ||
        (state.firstPlayer === "x" && !state.isXTurn);
...
export const { setFirstPlayer, startGame, restartGame, makeMove, newRound } =
  gameSlice.actions;
